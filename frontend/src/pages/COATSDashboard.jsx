...
export default function COATSDashboard() {
  ...
  const getTimelineParams = useCallback(() => {
    if (timelinePreset === "custom") {
      const p = new URLSearchParams();
      if (customDates.from) p.append("date_from", customDates.from);
      if (customDates.to) p.append("date_to", customDates.to);
      return p.toString();
    }
    const to = new Date();
    const from = new Date();
    if (timelinePreset === "3m") from.setMonth(from.getMonth() - 3);
    else if (timelinePreset === "6m") from.setMonth(from.getMonth() - 6);
    else if (timelinePreset === "1y") from.setFullYear(from.getFullYear() - 1);
    else return ""; // "all"
    
    return `date_from=${from.toISOString().split("T")[0]}&date_to=${to.toISOString().split("T")[0]}`;
  }, [timelinePreset, customDates]);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "1.5rem" }}>
          ...
          <Card t={t}>
            ...
            <div style={{ display:"flex", gap:10, marginBottom:15, alignItems:"center", flexWrap:"wrap" }}>
              <div style={{ display:"flex", background:t.bgCardHover, borderRadius:8, border:`1px solid ${t.border}`, padding:2 }}>
                {["3m", "6m", "1y", "all", "custom"].map(p => (
                  <button key={p} onClick={() => setTimelinePreset(p)}
                    style={{ 
                      padding:"4px 12px", background: timelinePreset===p ? t.accent : "transparent",
                      color: timelinePreset===p ? "#fff" : t.textSecond, 
                      border:"none", borderRadius:6, cursor:"pointer",
                      fontFamily:"'JetBrains Mono',monospace", fontSize:"0.65rem", fontWeight:600, textTransform:"uppercase", transition:"all .2s"
                    }}>
                    {p}
                  </button>
                ))}
              ...
              {timelinePreset === "custom" && (
                <div style={{ display:"flex", gap:5, alignItems:"center", animation:"cSlide .2s ease" }}>
                  <input type="date" value={customDates.from} onChange={e=>setCustomDates(prev=>({...prev, from:e.target.value}))}
                    style={{ background:t.bgBase, border:`1px solid ${t.border}`, color:t.textPrimary, padding:"3px 8px", borderRadius:6, fontSize:"0.7rem", fontFamily:"'JetBrains Mono',monospace" }} />
                  <span style={{ color:t.textMuted, fontSize:"0.7rem" }}>to</span>
                  <input type="date" value={customDates.to} onChange={e=>setCustomDates(prev=>({...prev, to:e.target.value}))}
                    style={{ background:t.bgBase, border:`1px solid ${t.border}`, color:t.textPrimary, padding:"3px 8px", borderRadius:6, fontSize:"0.7rem", fontFamily:"'JetBrains Mono',monospace" }} />
                </div>
              )}
            </div>
